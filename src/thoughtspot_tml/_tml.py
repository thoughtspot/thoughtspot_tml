...
class TML:
    def __init__(self, tml_ordereddict: ["typing.OrderedDict", Dict]):
        ...
        if tml_ordereddict.get("type") and tml_ordereddict.get("type").startswith(
            "RDBMS"
        ):  # have to do a special check for connection.
...
"""
    ...
    def create_worksheet_column(
        column_display_name: str,
        ws_table_path_id: str,
        ...
        column_template = OrderedDict(
            {
                "name": column_display_name,
                "column_id": column_id,
                "properties": {"column_type": column_type, "index_type": index_type},
            }
        )
...
class Table(TML):
    ...
    @staticmethod
    def generate_tml_from_scratch(
        connection_name: str, db_name: str, schema: str, db_table: str, table_name: str = None
    ):
        template = """
table:
  name: {name}
  ...
  columns:
        ...
        return template.format(
            connection_name=connection_name, name=table_name, db=db_name, schema=schema, db_table=db_table
        )
        ...
        column_template = OrderedDict(
            {
                ...
                "properties": {"column_type": column_type, "index_type": index_type},
                "db_column_properties": {"data_type": column_data_type},
            }
        )
...
class Pinboard(TML):
    ...
    def update_worksheet_on_all_visualizations_by_fqn(
        self, original_worksheet_name: str, new_worksheet_guid_for_fqn: str
    ):
        for a in self.visualizations:
            answer = Answer(a)
            answer.change_worksheet_by_fqn(
                original_worksheet_name=original_worksheet_name, new_worksheet_guid_for_fqn=new_worksheet_guid_for_fqn
            )
    ...
    def replace_worksheet_on_all_visualizations(self, new_worksheet_name: str, new_worksheet_guid_for_fqn: str):
        for a in self.visualizations:
            answer = Answer(a)
            answer.replace_worksheet(
                new_worksheet_name=new_worksheet_name, new_worksheet_guid_for_fqn=new_worksheet_guid_for_fqn
            )
